...
class MyNotes {
  ...
  events() {
    this.myNotes.addEventListener('click', '.delete-note', e => this.deleteNote(e));
    this.myNotes.addEventListener('click', '.edit-note', e => this.editNote(e));
    this.myNotes.addEventListener('click', '.update-note', e => this.updateNote(e));
  ...
  async createNote() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(data.root_url + '/wp-json/wp/v2/note/', newPost);
      if (response.data != 'Você atingiu o limite de criação de notas.') {
        ...
        document.querySelector('#myNotes').insertAdjacentHTML('afterbegin', `<li data-id="${response.id}" class="note-item">
      ...
      if (response.responseText == 'Você atingiu o limite de criação de notas.') {
  ...
  makeNoteReadOnly(thisNote) {
    thisNote.querySelector('.note-title').setAttribute('readonly', 'readonly').classList.add('form-control-plaintext resize-none').classList.remove('form-control');
    thisNote.querySelector('.note-body').setAttribute('readonly', 'readonly').classList.add('form-control-plaintext resize-none').classList.remove('form-control');
    thisNote.querySelector('.update-note').classList.add('d-none').classList.remove('d-block');
    thisNote.querySelector('.edit-note').html('Edit');
    thisNote.setAttribute('state', 'cancel');
  }
  makeNoteEditable(thisNote) {
    thisNote.querySelector('.note-title').removeAttribute('readonly').classList.add('form-control').classList.remove('form-control-plaintext resize-none');
    thisNote.querySelector('.note-body').removeAttribute('readonly').classList.add('form-control').classList.remove('form-control-plaintext resize-none');
    thisNote.querySelector('.update-note').classList.add('d-block').classList.remove('d-none');
  ...
  async deleteNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()({
        method: 'delete',
        url: data.root_url + '/wp-json/wp/v2/note/103' + thisNote.getAttribute('data-id')
      });
  ...
  async updateNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default()({
        method: 'post',
        url: data.root_url + '/wp-json/wp/v2/note/' + thisNote.getAttribute('data-id'),
        data: updatedPost
      });
      this.makeNoteReadOnly(thisNote);
    } catch (e) {
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(data.root_url + '/wp-json/al-advogados/v1/search?term=' + this.searchField.value);
      const results = response.data;
      this.resultsDiv.innerHTML = `
        <div class="row">
          <div class="col-4">
          ...
          <div class="col-4">
            <h2>Programs</h2>
            ${results.programs.length ? '<ul class="results-list">' : `<p>Não há programas deste tipo. <a href="${data.root_url}/programs">Veja todos os programas</a></p>`}
            ...
            <hr>
            <h2>Professors</h2>
            ${results.professors.length ? '<div classs="row">' : `<p>Não há professores correspondentes.</p>`}
              ${results.professors.map(item => `<div class="col-12 col-md-6 col-lg-3">
                      <div class="card mb-4">
                        <div class="card-body">
          ...
          <div class="col-4">
            <h2>Campuses</h2>
            ${results.campuses.length ? '<ul class="results-list">' : `<p>Não há nenhum campus de acordo com sua buscas. <a href="${data.root_url}/campuses">Veja todos os campi.</a></p>`}
            ...
            <hr>
            <h2>Events</h2>
            ${results.events.length ? '<ul class="results-list">' : `<p>Não há nenhum campus de acordo com sua buscas. <a href="${data.root_url}/events">Veja todos os eventos.</a></p>`}
              ${results.events.map(item => `<div class="col-12 col-md-6 col-lg-4">
                      <div class="card mb-4">
